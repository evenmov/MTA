...
function toggleJetPack()
	if not isPedWearingJetpack(g_Me) then
		server.setPedWearingJetpack(g_Me,true)
		guiCheckBoxSetSelected(getControl(wndMain, 'jetpack'), true)
	else
		server.setPedWearingJetpack(g_Me,false)
...
function updateGUI(updateVehicle)
	...
	guiCheckBoxSetSelected( getControl(wndMain, 'jetpack'), isPedWearingJetpack(g_Me) )
...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		...
		bindKey('F2', 'down', toggleFRWindow)
