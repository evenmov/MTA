...
aLastCheck = 0
aCurrentVehicle = 429
aCurrentWeapon = 30 --改为给予物品ID
aCurrentAmmo = 1 -- 改为给予物品数量
aCurrentSlap = 20
aPlayers = {}
aBans = {}
aLastSync = 0
...
y=y+B  aTab1.VehicleHealth	= guiCreateLabel ( 0.26, y, 0.25, 0.04, "Vehicle Health: 0%", true, aTab1.Tab )
		...
		aTab1.GiveWeapon		= guiCreateButton ( 0.71, 0.485, 0.27, 0.04, "给予: 物品", true, aTab1.Tab, "giveweapon" ) -- 给武器按钮
		...
		aTab1.WeaponOptions	= guiCreateGridList ( 0.71, 0.485, 0.27, 0.48, true, aTab1.Tab )
						  ...
						  --[[
						  for i=1,#items do
							local row = guiGridListAddRow ( aTab1.WeaponOptions );
							guiGridListSetItemText ( aTab1.WeaponOptions, row, 1, Loc:Loc(items[i][1]), false, false )--
							guiGridListSetItemData ( aTab1.WeaponOptions, row, 1, i )
						  end
						  ]]
						  
						  for i = 1, 46 do
							if ( getWeaponNameFromID ( i ) ~= false ) then
								guiGridListSetItemText ( aTab1.WeaponOptions, guiGridListAddRow ( aTab1.WeaponOptions ), 1, getWeaponNameFromID ( i ), false, false ) 
							end
							end
...
function aClientDoubleClick ( button )
	if ( source == aTab1.WeaponOptions ) then
		...
		if ( guiGridListGetSelectedItem ( aTab1.WeaponOptions ) ~= -1 ) then
			aCurrentWeapon = guiGridListGetItemData ( aTab1.WeaponOptions, guiGridListGetSelectedItem ( aTab1.WeaponOptions ), 1 )
			...
			guiSetText ( aTab1.GiveWeapon, "给予: "..wep.." " )
		end
		guiSetVisible ( aTab1.WeaponOptions, false )
